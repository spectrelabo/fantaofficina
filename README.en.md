...
Spectre - FantaMoneyball uses a statistical model to estimate an expected points range
(pessimistic / median / optimistic) for each player, taking injuries and recent form into
account. The fair price is derived by comparing each player's expected value against the
rest of the market.

The actual predictive engine is a separate, proprietary package (`fanta_lab_engine`).
Without it, the app still works using demo values from `examples/dataset_sample.csv` —
useful for exploring the interface and the methodology, not for use in a real league.
